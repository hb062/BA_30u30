...
- Weitere Teilnetzwerke
  - Berufliche Stationen
    ...
    - Rolle der Sponsoren / #30u3-Partnerunternehmen
      ...
      - Vergleich der einzelnen Jahrgänge
...
6. Wie viele der Mitglieder waren Stipendiat:innen?
...
```{r Frauennetzwerk: Plot - Populärste Organisationen}
...
Outdegree-Verteilung: Berechnet und sortiert die Outdegree-Werte der männlichen Mitglieder von #30u30.
...
# Labelanpassung
V(members_preentry2)$label <- ifelse(V(members_preentry2)$type==2, V(members_preentry2)$name, NA) # Labels nur bei Organisationen anzeigen
V(members_preentry2)$label <- ifelse(V(members_preentry2)$betweenness > 52000, V(members_preentry2)$label, NA) # Labels nur bei Knoten mit Betweenness-Wert über 5200 anzeigen
```
...
Constraint-Werte im Arbeitsnetzwerk bis März 2022: Wie redundant sind die Beziehungen der #30u30?

1. Berechne constraint-Werte für alle Knoten des Type 1 (Personen)
```{r Berufliche Stationen - Bis März 2022: Constraint}
constraint(work_simple, V(work_simple)[type == 1])
```
2. Berechne den Mittelwert der constraint-Werte für #30u30
```{r Berufliche Stationen - Bis März 2022: Constraint-Mittelwert}
constr_work <- constraint(work_simple, V(work_simple)[type == 1])
constr_work
...
plot(scholars_simple,
     ...
     vertex.label=ifelse(ind_scholars>2, V(scholars_simple)$name, NA),
     vertex.label.degree=0, 
     vertex.label.cex=ind_scholars*.5,
     vertex.label.family="sans",
     vertex.label.font=2,
     vertex.label.color="black", 
     asp=0,
...
# Dichte
edge_density(nonscholars_simple) # 0,0015
edge_density(scholars_simple) # 0,0035
```
...
Wie redundant sind die Kontakte der (Nicht-)Stipendiat*innen? Berechne constraint:
```{r Stipendiat:innen und Nicht-Stipendiat:innen: constraint}
# # im Stip.-Netzwerk:
constr_schol <- constraint(scholars_simple, V(scholars_simple)[type == 1]) 
mean(constr_schol, na.rm=T) # 0.09302989

# im Nicht-Stip.-Netzwerk:
constr_nschol <- constraint(nonscholars_simple, V(nonscholars_simple)[type == 1]) 
mean(constr_nschol, na.rm=T) # 0.1542615
```
...
Plottet das Mentor:innen-Netzwerk (Knotengröße entspr. dem Indegree-Wert skaliert):
...
Erstellt Subgraph der Mentor:innen mit entspr. Verbindungen:

```{r Teilnetzwerk Mentor:innen - Mitgliedschaft Branchenverbände}
mentors
mentor_mem <- subgraph.edges(mentors, E(mentors)[relationship == 11]) 
# selektiert Kanten, bei denen es sich um Mitgliedschaft handelt
mentor_mem2 <- add.edges(mentor_mem, E(mentor_mem)[relationship == 13]) 
# selektiert Kanten, bei denen es sich um leitende Pos. in Vereinen handelt
...
Plottet das Netzwerk der Mentor:innen, die in Branchenverbänden aktiv sind:

```{r Teilnetzwerk Mentor:innen - Mitgliedschaft Branchenverbände - Plot}
plot(mentor_mem2,
     layout=layout_with_fr,
...
```{r Teilnetzwerk Mentorship erstellen}
g 
...
Generiert den Plot der Mentoringbeziehungen:

```{r Teilnetzwerk Mentorship - Plot}
...
Gibt es Mentor:innen, die mit mehreren #30u30-Mitgliedern in Verbindung stehen? Liefert Indegree-Verteilung:

```{r Teilnetzwerk Mentoring - Indegree-Verteilung}
ind_mentoring
sort(ind_mentoring, decreasing = T) 
```

Welche Mentees haben besonders viele Mentor:innen?
Liefert Outdegree-Werte:
```{r Teilnetzwerk Mentoring - Outdegree-Verteilung}
...
#### Teil-/Ego-Netzwerke Mentor:innen und Mentees

##### Ego-Netzwerk Mentor: Andreas Winiarski
...
```{r Teilnetzwerk Andreas Winiarski und Mentees erstellen}
# Einlesen der Edgelist
...
Färbt verschiedene Knoten unterschiedlich ein:
```{r Teilnetzwerk Andreas Winiarski und Mentees - Visualisierungsanpassung}
...
Plottet das neu generierte Netzwerk von Winiarski und seinen Mentees:
```{r Teilnetzwerk Andreas Winiarski und Mentees - Plot}
...
```{r Ego-Netzwerk Judith Götter und Mentor:innen erstellen}
g
...
```{r Ego-Netzwerk Judith Götter und Mentor:innen - Plot}
plot(goetter[[1]], 
